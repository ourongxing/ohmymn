...
import { getSelectNodes, removeHighlight } from "utils/note"
...
const configs: IConfig<IProfile["export2devonthink"], typeof ActionKey> = {
  ...
  settings: [
    ...
    },
    {
      ...
      bind: [["showTemplate", 1],["exportMethod",1]]
    },
    {
      type: CellViewType.Input,
      key:"pdfsource",
      help:"URL用于下载为PDF文件(请务必加上http或者https)",
      bind:[["showTemplate",1],["exportMethod",0]]
    },
    {
      type: CellViewType.Input,
      key: "mdtext",
      help: "正文文本",
      bind: [["showTemplate", 1],["exportMethod",2]]
    },
    {
      type: CellViewType.Input,
      key: "txtext",
      help: "正文文本",
      bind: [["showTemplate", 1],["exportMethod",3]]
    },
    {
      type: CellViewType.Input,
      key: "hide",
...
const utils = {
  getDtContent : (node:MbBookNote,content_list:string[],content_key:string[],addTags:number[],tags:string) =>{
    const content_final = content_list.reduce(
      (acc, cur) => {
        if (cur) {
          ...
          if(c){acc.push(escapeURLParam(c))}
          else{acc.push("")}
        }
        else
        {acc.push("")}
  ...
  async getContent(node: MbBookNote, option: number) {
    ...
    if (self.profile.export2devonthink.exportMethod[0]==0){
      const content_list = [title,comment,destination,pdfsource,hide,referrer,width,paginated]
      const content_key = ["title","comment","destination","location","hide","referrer","width","paginated"]
      const output = utils.getDtContent(node,content_list,content_key,addTags,tags)
      return "x-devonthink://createPDF?"+output
    }
    else if(self.profile.export2devonthink.exportMethod[0]==1){
      const content_list = [title,comment,destination,htmlsource,hide,referrer]
      const content_key = ["title","comment","destination","location","hide","referrer"]
      const output = utils.getDtContent(node,content_list,content_key,addTags,tags)
      return "x-devonthink://createHTML?"+output
    }
    else if(self.profile.export2devonthink.exportMethod[0]==2){
      console.log([title,comment,destination,mdtext,hide,referrer],"export2devonthink")
      const content_list = [title,comment,destination,mdtext,hide,referrer]
      const content_key = ["title","comment","destination","text","hide","referrer"]
      const output = utils.getDtContent(node,content_list,content_key,addTags,tags)
...
const checker: ICheckMethod<PickByValue<IProfile["export2devonthink"], string>> = (
  input,
  key
) => {
  switch (key) {
