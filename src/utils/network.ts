class Response {
    ...
    json(): any {
        const json = NSJSONSerialization.JSONObjectWithDataOptions(this.data, NSJSONReadingOptions.MutableContainers)
        if (json) return json
        // new Error 不能使用
        throw "不能序列化为 JSON"
    }
...
const initRequest = (url: string, options: RequestOptions): NSMutableURLRequest => {
    ...
    if (options.body)
        request.setHTTPBody(NSJSONSerialization.dataWithJSONObject(options.body, NSJSONWritingOptions.SortedKeys))
    return request
}
...
const fetch = (url: string, options: RequestOptions = {}): Promise<Response> => {
    return new Promise((resolve, reject) => {
        ...
        NSURLConnection.sendAsynchronousRequestQueueCompletionHandler(request, queue, (res: NSHTTPURLResponse, data: NSData, error: NSError) => {
